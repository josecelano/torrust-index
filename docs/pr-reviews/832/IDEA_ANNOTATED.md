...
> 📝 **Annotation:** φ is the Greek letter **phi**, pronounced **"fye"** in English mathematics. Full title reads: **"fye-Bounded Geometric-Value Graph"**.
...
The G-V Graph continuously solves a rate-distortion allocation problem.

> 📝 **Annotation — "rate-distortion allocation problem":** This is an information-theory concept:
>
> - **Rate** = the resources spent on the representation — here, the number of G-nodes / tree depth.
> - **Distortion** = the inaccuracy from using a coarse interval instead of resolving a region finely.
> - **The trade-off:** You have a fixed node budget. Spend it on fine resolution in a few places, or coarse coverage everywhere. Rate-distortion theory asks: _given a budget, where do you spend it to minimise total error?_
>
> The G-V Graph continuously re-answers this question as observations arrive. When a region accumulates high significance, the V-Tree promotes it → G-node splits → more resolution there. When a region goes quiet, it eventually gets evicted → resolution freed up elsewhere. The system constantly reallocates its "bit budget" toward where the data is concentrated.
>
> **"Continuously"** means this is not a one-shot optimisation — it adapts dynamically with every `observe()` call.
>
> Plain English: _"The system constantly decides where in the domain to spend its limited resolution budget, allocating more precision to active regions and less to quiet ones."_

The geometric tree maintains a dynamic base-2 description code — a Kraft equality $\sum 2^{-d_i} = 1$ over the spatial domain, allocating precision toward observed structure, not away from it: the dual of source coding. The value tree governs this allocation through competitive ranking: the max-uncle constraint (§1.5.3) implies at least Fibonacci-rate decay along root-to-leaf paths, bounding the governance overhead at $1/\log_2\varphi \approx 1.44$ times Shannon entropy. The Fibonacci recurrence (§18.1) proves the bound tight (§18.3). The G-Tree is the code. The V-Tree is the policy that shapes it. $\varphi$ lives in the policy's efficiency — in how quickly the system identifies where the code's precision budget should next be spent — not in the code's structure.

> 📝 **Annotation — full paragraph breakdown:**
>
> **"maintains a dynamic base-2 description code"** — The G-Tree is a binary tree (base-2 = every split divides in half). Its shape defines a partition of the domain into intervals — this partition _is_ the "code" (a codebook of prefix-free codewords, as explained below).
>
> > 📝 **Background — codebook, prefix-free codewords, and Kraft equality:**
> >
> > **Codebook:** A lookup table mapping each symbol to a binary string. Example encoding A, B, C, D:
> >
> > | Symbol | Code |
> > | ------ | ---- |
> > | A      | `00` |
...
> > | D      | `11` |
> >
> > Each row is a **codeword**. The full table is the **codebook**.
> >
> > **Prefix-free:** A code where no codeword is the start of another codeword. This lets you decode a stream of bits unambiguously without separators. A non-prefix-free example (bad): A=`0`, B=`01` — does `01` mean "B" or "A then something"? A prefix-free example (good): A=`0`, B=`10`, C=`110`, D=`111` — no codeword starts another.
> >
> > **The key insight:** a prefix-free code corresponds exactly to a binary tree where symbols sit at the leaves. Going left = `0`, going right = `1`. Internal nodes are just routing. The tree _is_ the code:
> >
> > ```
> >         root
> >        /    \
...
> > ```
> >
> > In the G-Tree, the domain intervals are the "symbols" and the leaf nodes are the "codewords" — the tree shape is the codebook.
> >
> > **Kraft equality** $\sum 2^{-d_i} = 1$: Think of each node as owning a fraction of the domain. The root owns fraction 1 (everything). Each split divides a node's fraction in half. Each leaf at depth $d$ owns fraction $2^{-d}$. A complete tree (every internal node has exactly 2 children) always tiles the whole domain perfectly — no gaps, no overlaps — so the fractions sum to 1.
> >
> > Example tree with leaves at depths 1, 2, 2:
> >
> > ```
> >          root  (fraction = 1)
> >         /     \
> >       [0,4)   [4,8)     depth 1, fraction = 0.5
> >       /   \
> >   [0,2) [2,4)           depth 2, fraction = 0.25 each
> > ```
> >
> > $2^{-2} + 2^{-2} + 2^{-1} = 0.25 + 0.25 + 0.5 = 1$ ✓
> >
> > The G-Tree always satisfies this because every split creates exactly 2 children, so the leaves always perfectly cover the domain no matter how many splits and evictions have happened.
>
> **"a Kraft equality $\sum 2^{-d_i} = 1$"** — see background annotation above.
>
> **"allocating precision toward observed structure, not away from it: the dual of source coding"** — Normal compression (e.g. Huffman) assigns short codes to _frequent_ things to save space. The G-Tree does the opposite: it assigns deep nodes (= longer codes = more precision) to _active_ regions to resolve them more finely. Same tool, opposite goal — that is why it is called the "dual".
>
> **"The value tree governs this allocation through competitive ranking"** — The V-Tree decides which regions deserve more depth (more precision). It does this by making G-nodes compete for significance — the most significant ones get promoted and eventually split, gaining more resolution.
>
> **"bounding the governance overhead at $1/\log_2\varphi \approx 1.44$ times Shannon entropy"** — Shannon entropy is the theoretical _minimum_ cost to describe a distribution — the absolute best any system could do. The V-Tree's overhead is at most 1.44× that minimum, i.e. at most ~44% worse than theoretically optimal.
>
> **"The G-Tree is the code. The V-Tree is the policy that shapes it."** — Two-line summary of the whole system: G-Tree = _what_ the current resolution partition looks like; V-Tree = _why_ it looks that way (the decisions about where to spend resolution).
>
> **"$\varphi$ lives in the policy's efficiency — not in the code's structure"** — φ (≈1.618) appears only in the V-Tree's depth bound (because the tournament rules produce Fibonacci-rate decay, and Fibonacci numbers grow as φⁿ). The G-Tree is just a plain binary tree — φ has nothing to do with its shape.

> 📝 **Annotation — "Fibonacci-rate decay":** The max-uncle constraint implies that importance must decrease along every root-to-leaf path in the V-Tree.
>
> _(What is "importance"? Each G-node carries a numeric accumulator called its **importance** — a user-defined measure of how significant that node's region is. Every time an observation `(coord, delta)` lands on a node, the node's importance is updated via the projection function π (e.g. `importance += delta` in the standard config). So importance is essentially a running score: how much activity has this spatial region received? Nodes with high importance rank near the top of the V-Tree; nodes with low importance are pushed toward the leaves or evicted. It is distinct from `sum` — `sum` counts everything accumulated in a region including all descendants, while `importance` counts only what arrived at that specific node's scale.)_
>
> _(Why not call it "density"? "Density" would imply normalization by interval size — observations per unit area. But importance is **not** normalized: a wide region and a narrow region that both received 100 units of observation have the **same** importance but very different density. More importantly, importance is deliberately **opaque and user-defined** — the architecture doesn't care what it means semantically, only how to compare, aggregate, and accumulate it. You could plug in raw activity volume, absolute activity, or a fully custom metric. "Density" would fix a specific semantic. "Importance" leaves it open. A clearer informal substitute might be **"score"** or **"significance"**.)_ In the worst case (tightest packing), the minimum number of nodes needed to support a subtree of depth $d$ follows the Fibonacci recurrence $F(d) = F(d-1) + F(d-2)$ — the same argument that proves AVL trees have $O(\log_\varphi n)$ height. Since $F(d) \sim \varphi^d/\sqrt{5}$, importance along a path must decay at least as fast as $\varphi^{-d}$: each level down it shrinks by at least a factor of $\varphi \approx 1.618$. This bounds the V-Tree depth to $\log_\varphi(\text{total importance})$, which is $1/\log_2\varphi \approx 1.44$ times the Shannon-optimal depth — i.e. at most ~44% overhead.

> 📝 **Annotation — "The G-Tree is the code":** "Code" here means an **information-theory description code**, not program code. The Kraft equality $\sum_i 2^{-d_i} = 1$ (where $d_i$ is the depth of each contour leaf) is exactly the condition that defines a **complete prefix-free binary code** — like a Huffman codebook. Shallower nodes = shorter codewords = coarser intervals; deeper nodes = longer codewords = finer resolution. The contour is the codebook. The twist vs. Huffman: Huffman assigns short codes to _frequent_ symbols to compress; the G-Tree assigns deep nodes to _high-observation_ regions to _resolve_ them — the dual direction.
